...
export default async function LegislationPage({params}:{params:Promise<{slug:string}>}) { const {slug}=await params; const event=eventBySlug(slug); if(!event||event.category!=='Laws & Bills') notFound(); const recordSources=expandedSourcesFor(event); return <DetailShell party={event.party} eyebrow="Legislation" title={event.title} summary={event.summary} date={event.date} evidence={`${recordSources.length} primary or institutional records`} sources={recordSources}><LongformRecordArticle event={event} sources={recordSources}/></DetailShell>; }
