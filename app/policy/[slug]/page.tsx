...
export default async function PolicyPage({params}:{params:Promise<{slug:string}>}) { const {slug}=await params; const event=eventBySlug(slug); if(!event||!['Policies','Social Policy','Infrastructure'].includes(event.category)) notFound(); const recordSources=expandedSourcesFor(event); return <DetailShell eyebrow={event.category} title={event.title} summary={event.summary} date={event.date} evidence={`${recordSources.length} primary or institutional records`} sources={recordSources}><LongformRecordArticle event={event} sources={recordSources}/></DetailShell>; }
